...
impl Hasher {
    ...
    pub fn hash_blake2s_simd(&mut self) {
        let _hash = irox_simd::blake2::BLAKE2s256::default().hash(&self.iter);
        self.iter[0] += 1;
    }
...
pub fn bench_blake2s_file(path: &str) {
    ...
    let mut buf = <Box<[u8]> as ZeroedBuffer>::new_zeroed(32 * 1024);
    loop {
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    grp.finish();
    // std::thread::sleep(Duration::from_secs(20));
    let mut grp = c.benchmark_group("blake2s256_file");
